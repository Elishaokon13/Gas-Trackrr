...
/**
 * Helper to fetch historical price from CoinGecko
 * @param {string} coinId - CoinGecko coin id (e.g. 'ethereum', 'usd-coin')
 * @param {string} date - 'dd-mm-yyyy'
 * @returns {Promise<number>} - price in USD
 */
async function fetchCoinGeckoPrice(coinId, date) {
...
/**
 * Fetch daily ETH and USDC balances and USD value for a given address and date range using Alchemy
 * @param {string} address - Wallet address
 ...
 * @returns {Promise<Array>} - Array of { date, eth, usdc, usd }
 */
export async function getHistoricalBalances({ address, startDate, endDate, alchemyApiKey, usdcAddress, alchemyNetwork }) {
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    // Fetch historical prices from CoinGecko (format: dd-mm-yyyy)
    ...
    const [ethPrice, usdcPrice] = await Promise.all([
      fetchCoinGeckoPrice('ethereum', cgDate),
      fetchCoinGeckoPrice('usd-coin', cgDate)
    ]);
    // Calculate USD value
