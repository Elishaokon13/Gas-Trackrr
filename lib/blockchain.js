...
/**
 * Calculate gas spent from transaction history
 * @param {Array} transactions - Transaction history
 * @param {string} address - Wallet address
 * @returns {Object} - Gas spent in ETH and wei
 */
function calculateGasSpent(transactions, address) {
  console.log(`Calculating gas for ${transactions.length} transactions`);
  ...
  console.log(`Total gas spent: ${totalGasEth} ETH (${outgoingCount} outgoing txs)`);
...
/**
 * Get wallet data including transaction count and gas spent
 * @param {string} address - Wallet address
...
export const getWalletData = async (address) => {
  try {
    ...
    // Calculate gas spent
    const gasInfo = calculateGasSpent(txHistory, normalizedAddress);
    
    // Calculate USD value
    const gasUsdAmount = (parseFloat(gasInfo.ethAmount) * ethPrice).toFixed(2);
    ...
    return {
      ...
      outgoingTransactions: gasInfo.outgoingCount,
      gasSpent: {
        ethAmount: gasInfo.ethAmount,
        weiAmount: gasInfo.weiAmount,
