...
export const getZerionTransactions = async (address) => {
  if (!process.env.NEXT_PUBLIC_ZERION_API_KEY) {
    console.log('No Zerion API key found, skipping Zerion API call');
    return null;
  }
  
  try {
    console.log(`Getting transactions from Zerion API for ${address}...`);
    const headers = getZerionHeaders();
    console.log('Using Zerion headers:', JSON.stringify(headers).replace(/"authorization":"[^"]+"/g, '"authorization":"Basic ***"'));
    
    const response = await axios.get(
      `${ZERION_API_URL}/wallets/${address}/transactions?currency=usd&chain_ids=base&page=1&sort=datetime&limit=50`,
      { headers: headers, timeout: 20000 }
    );
    
    const transactions = response.data?.data || [];
    console.log(`Found ${transactions.length} transactions via Zerion API`);
    
    return transactions;
  } catch (error) {
    console.error('Error fetching transactions from Zerion:', error.response?.status, error.response?.statusText);
...
// Add gas expense tracking functions
...
export const getMonthlyGasExpenses = async (transactions) => {
  ...
  if (transactions[0]?.type === 'transactions') {
    ...
    try {
      ...
      console.warn('Could not fetch ETH price, using default:', error.message);
    }
...
// Update getWrappedData to include the new gas expense tracking
