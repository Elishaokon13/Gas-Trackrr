...
const BASENAME_CONTRACT = '0x3f3f20dff9350851d9b121b9b7b1f185518d5f1c';
 ...
 * @returns {boolean} - Whether the name is a valid BaseName
 */
function isValidBaseName(name) {
  // BaseName format: alphanumeric + .base or .base.eth
 ...
 * @param {string} name - The BaseName to normalize
 ...
 * @param {string} baseName - The BaseName to resolve
...
async function resolveBaseName(baseName) {
  try {
    ...
    // Check if it's a valid BaseName format
    ...
    console.log(`Normalized BaseName: ${normalizedName}`);
    ...
    const contract = new ethers.Contract(BASENAME_CONTRACT, abi, provider);
    
    // Get the token ID from the name
    const nameHash = ethers.utils.keccak256(ethers.utils.toUtf8Bytes(normalizedName));
    const tokenId = ethers.BigNumber.from(nameHash).mod(ethers.BigNumber.from(2).pow(256));
    
    try {
      const owner = await contract.ownerOf(tokenId);
      console.log(`Resolved ${baseName} to ${owner}`);
      return owner;
    } catch (error) {
      console.log(`No owner found for ${baseName}`);
      return null;
    }
  } catch (error) {
