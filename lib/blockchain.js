...
/**
 * Get the total transaction count for an address using Basescan API
 * @param {string} address - The wallet address
 * @returns {Promise<number>} - Number of transactions
 */
export const getTransactionCount = async (address) => {
  try {
    logMessage('info', `Getting transaction count for ${address}`);
    ...
    if (response.data.status === '1') {
      // If successful, the result will include a count
      const count = parseInt(response.data.result.length);
      logMessage('info', `Found ${count} transactions`);
      return count;
    } else {
      // If error or no transactions
      logMessage('info', `No transactions found or API error: ${response.data.message}`);
      return 0;
    }
  } catch (error) {
    logMessage('error', 'Error fetching transaction count:', error.message);
    return 0;
  }
};

/**
 * Calculate total gas spent by an address using Basescan API
 * @param {string} address - The wallet address
 * @returns {Promise<{ethAmount: string, weiAmount: string}>} - Gas spent in ETH and wei
 */
export const getTotalGasSpent = async (address) => {
  try {
    logMessage('info', `Getting gas spent for ${address}`);
    
    // Normalize address
    const normalizedAddress = ethers.utils.getAddress(address);
    
    // Make API request to Basescan - we need all transactions
    const response = await axios.get(BASESCAN_API_URL, {
      params: {
        module: 'account',
        action: 'txlist',
        ...
        endblock: '99999999',
        page: '1',
        offset: '10000',  // Get up to 10,000 transactions
        sort: 'asc',
        apikey: BASESCAN_API_KEY
      }
    });
    ...
    if (response.data.status === '1' && response.data.result.length > 0) {
      ...
      logMessage('info', `Processing ${transactions.length} transactions for gas calculation`);
      
      // Calculate total gas spent
      let totalGasWei = ethers.BigNumber.from(0);
      
      for (const tx of transactions) {
        // Only count transactions sent by this address (not received)
      ...
      logMessage('info', `Total gas spent: ${totalGasEth} ETH (${totalGasWei.toString()} wei)`);
    ...
    logMessage('error', 'Error calculating gas spent:', error.message);
    return {
      ethAmount: '0',
      weiAmount: '0'
    };
...
/**
 * Get wallet data - transaction count and gas spent
 * @param {string} address - The wallet address
 * @returns {Promise<Object>} - Wallet data
 */
export const getWalletData = async (address) => {
  try {
    logMessage('info', `Getting wallet data for ${address}`);
    
    // Get data in parallel for better performance
    const [txCount, gasSpent] = await Promise.all([
      getTransactionCount(address),
      getTotalGasSpent(address)
    ]);
    ...
    logMessage('error', 'Error getting wallet data:', error.message);
    throw error;
  }
}; 