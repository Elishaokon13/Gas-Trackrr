...
export const getWalletData = async (addressOrName, chain = 'base') => {
  try {
    ...
    if (chain === 'assetchain') {
      ...
      // Calculate metrics (implement similar to other chains, using txs)
      // For now, return basic info
      return {
        address: normalizedAddress,
        baseName: null,
        transactionCount: txs.length,
        outgoingTransactions: txs.filter(tx => tx.from && tx.from.toLowerCase() === normalizedAddress.toLowerCase()).length,
        ethVolumeIn: '0',
        ethVolumeOut: '0',
        usdcVolumeIn: '0',
        usdcVolumeOut: '0',
