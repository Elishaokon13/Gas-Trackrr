...
export const getWalletData = async (addressOrName, chain = 'base') => {
  try {
    ...
    if (chain === 'assetchain') {
      ...
      const usdcAddress = USDC_ADDRESSES.assetchain.toLowerCase();
      ...
      // Get RWA price in USD (optional, fallback to 1)
      let rwaPrice = 1;
      try {
        const resp = await axios.get('https://api.coingecko.com/api/v3/simple/price?ids=realio-network&vs_currencies=usd');
        if (resp.data?.['realio-network']?.usd) rwaPrice = resp.data['realio-network'].usd;
      } catch {}
      ...
      const gasUsd = (parseFloat(gasEth) * rwaPrice).toFixed(2);
      return {
        address: normalizedAddress,
        baseName: null,
        ...
        ethVolumeInUsd: (parseFloat(rwaInEth) * rwaPrice).toFixed(2),
        ethVolumeOutUsd: (parseFloat(rwaOutEth) * rwaPrice).toFixed(2),
