...
const zerionHeaders = {
  ...
  'authorization': process.env.NEXT_PUBLIC_ZERION_API_KEY 
    ? `Basic ${process.env.NEXT_PUBLIC_ZERION_API_KEY}`
    : ''
};
...
export const getZerionTransactions = async (address) => {
  ...
  try {
    ...
    const response = await axios.get(
      ...
      { headers: zerionHeaders, timeout: 20000 }
    );
    ...
    console.error('Error fetching transactions from Zerion:', error);
...
export const getZerionNFTs = async (address) => {
  ...
  try {
    ...
    console.error('Error fetching NFTs from Zerion:', error);
    return null;
  }
};
