...
async function resolveEnsName(name) {
  try {
    ...
    // Use Universal Resolver contract
    const universalResolver = new ethers.Contract(
      '0xc0497E381f536Be9ce14B0dD3817cBcAe57d2F62', // Universal Resolver address
      ENS_RESOLVER_ABI,
      provider
    );
    ...
    const data = ethers.utils.defaultAbiCoder.encode(['bytes', 'bytes'], [nameBytes, '0x']);
    
    // Call resolve
    const [resolvedAddress] = await universalResolver.resolve(nameBytes, data);
...
export const getWalletData = async (addressOrName, chain = 'base') => {
  try {
    ...
    if (chain === 'ethereum' && typeof addressOrName === 'string' && addressOrName.endsWith('.eth')) {
      ...
      if (chain === 'base') baseName = await getBaseName(normalizedAddress);
    }
