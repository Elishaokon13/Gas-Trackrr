...
export const getTransactionHistory = async (address) => {
  try {
    logMessage('info', `Fetching transaction history for ${address}`);
    ...
    logMessage('info', `Basescan API response status: ${response.data.status}`);
    logMessage('info', `Basescan API message: ${response.data.message}`);
    ...
    if (response.data.message === 'NOTOK') {
      throw new Error(`Basescan API error: ${response.data.result}`);
    }

    throw new Error(`Failed to fetch transaction history: ${response.data.message}`);
  } catch (error) {
    logMessage('error', `Error in getTransactionHistory: ${error.message}`);
    if (error.response) {
      logMessage('error', `API Response: ${JSON.stringify(error.response.data)}`);
    }
    throw error;
  }
};
...
// Calculate gas spent
export const calculateGasSpent = async (address) => {
  try {
    const transactions = await getTransactionHistory(address);
    const gasSpent = transactions.reduce((total, tx) => {
      const gasUsed = parseInt(tx.gasUsed);
      const gasPrice = parseInt(tx.gasPrice);
      return total + (gasUsed * gasPrice);
    }, 0);
    
    return gasSpent / 1e18; // Convert from Wei to ETH
  } catch (error) {
...
// Calculate volume
export const calculateVolume = async (address) => {
  try {
    logMessage('info', `Calculating volume for ${address}`);
    
    const transactions = await getTransactionHistory(address);
    let incomingVolume = 0;
    let outgoingVolume = 0;

    logMessage('info', `Processing ${transactions.length} transactions for volume calculation`);

    transactions.forEach((tx, index) => {
      try {
        const value = parseInt(tx.value) / 1e18; // Convert from Wei to ETH
        ...
        logMessage('warning', `Error processing transaction ${index}: ${txError.message}`);
      }
    });
    ...
    const result = {
      ...
      total: incomingVolume + outgoingVolume
    };

    logMessage('info', `Volume calculation complete:`, result);
    return result;
  } catch (error) {
    logMessage('error', `Error in calculateVolume: ${error.message}`);
    throw error;
  }
};

/**
 * Calculate total incoming and outgoing transaction volume and gas spent from transaction history
...
function calculateVolumeAndGas(transactions, address) {
  ...
  let outgoingVolumeWei = ethers.BigNumber.from(0);
  let incomingVolumeWei = ethers.BigNumber.from(0);
  let outgoingCount = 0;
  ...
  for (const tx of transactions) {
    ...
    // Incoming: this address is the recipient
    if (tx.to && tx.to.toLowerCase() === normalizedAddress) {
      if (tx.value) {
        const txValue = ethers.BigNumber.from(tx.value || '0');
        incomingVolumeWei = incomingVolumeWei.add(txValue);
      }
    }
  }
  
  const totalGasEth = ethers.utils.formatEther(totalGasWei);
  const outgoingVolumeEth = ethers.utils.formatEther(outgoingVolumeWei);
  const incomingVolumeEth = ethers.utils.formatEther(incomingVolumeWei);
  
  return {
    gasEthAmount: totalGasEth,
    gasWeiAmount: totalGasWei.toString(),
    outgoingVolumeEthAmount: outgoingVolumeEth,
    outgoingVolumeWeiAmount: outgoingVolumeWei.toString(),
    incomingVolumeEthAmount: incomingVolumeEth,
    incomingVolumeWeiAmount: incomingVolumeWei.toString(),
    outgoingCount
  };
}
...
export const getWalletData = async (address) => {
  try {
    ...
    const outgoingVolumeUsdAmount = (parseFloat(stats.outgoingVolumeEthAmount) * ethPrice).toFixed(2);
    const incomingVolumeUsdAmount = (parseFloat(stats.incomingVolumeEthAmount) * ethPrice).toFixed(2);
    
    console.log(`Gas spent in USD: $${gasUsdAmount} (ETH price: $${ethPrice})`);
    console.log(`Outgoing volume in USD: $${outgoingVolumeUsdAmount}`);
    console.log(`Incoming volume in USD: $${incomingVolumeUsdAmount}`);
    ...
    return {
      ...
      outgoingVolume: {
        ethAmount: stats.outgoingVolumeEthAmount,
        weiAmount: stats.outgoingVolumeWeiAmount,
        usdAmount: outgoingVolumeUsdAmount
      },
      incomingVolume: {
        ethAmount: stats.incomingVolumeEthAmount,
        weiAmount: stats.incomingVolumeWeiAmount,
        usdAmount: incomingVolumeUsdAmount
      },
      ethPrice,
      success: true
    };
